...
class Mixformer(nn.Module):
    ...
    def forward(self, x):
        # B = x.size(0)
        # 右侧分支：先做depthwise，后做DPRNN
        intra_depthwise_tomix = self.depthwise(x)
        intra_depthwise_tomix = self.DPRNN1(intra_depthwise_tomix)  # （1，80，81，50）

        # 左侧分支：先做DPRNN后做attention
        # inter_out = x
        inter_out = intra_depthwise_tomix
        ...
        out = self.out_norm(inter_out_mixout)  ##(1,80,81,50)
        ...
        # out = out + intra_depthwise_mixout + feed_out  # (1,80,81,50)
        out = out.contiguous()
        return out
