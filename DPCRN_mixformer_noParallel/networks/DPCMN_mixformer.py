...
class Mixformer(nn.Module):
    ...
    def forward(self, x):
        ...
        intra_depthwise_tomix = intra_depthwise_tomix.contiguous().permute([0, 2, 1, 3])
        intra_depthwise_tomix = intra_depthwise_tomix.reshape(
            intra_depthwise_tomix.shape[0] * intra_depthwise_tomix.shape[1],
            intra_depthwise_tomix.shape[2], intra_depthwise_tomix.shape[3])
        value = self.value_norm(intra_depthwise_tomix)
        query = self.query_norm(intra_depthwise_tomix)
        intra_depthwise_tomix = self.attention_conv(query, value)

        intra_depthwise_tomix = intra_depthwise_tomix.contiguous().view(B, -1, intra_depthwise_tomix.shape[1],
                                                                        intra_depthwise_tomix.shape[2])
        intra_depthwise_tomix = intra_depthwise_tomix.permute([0, 2, 1, 3])
...
if __name__ == '__main__':
    ...
    #
    # # 创建SummaryWriter，指定日志保存路径
    # writer = SummaryWriter('logs')
    #
    # # 创建一个虚拟输入，用于生成计算图
    # dummy_input = torch.randn(1, 16, 16000)  # 根据你的模型输入形状调整
    #
    # # 创建模型
    # model = DPCMN_mixformer()
    #
    # # 将模型的计算图写入TensorBoard
    ...
    model = DPCMN_mixformer()
    input = (2, 16, 16000)
    input2 = torch.randn(2, 16, 16000).requires_grad_(True)  # 定义一个网络的输入值

    print(model)
    # summary(model, input)
    y = model(input2)  # 获取网络的预测值
    MyConvNetVis = make_dot(y, params=dict(list(model.named_parameters()) + [('x', input2)]))
    MyConvNetVis.format = "png"
    # 指定文件生成的文件夹
    MyConvNetVis.directory = "data"
